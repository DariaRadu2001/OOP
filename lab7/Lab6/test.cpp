...
void loschen_test() {
    ...
    //assert(repo.loschen_auto(Auto("CX-3", "Mazda", 2019, 50000, 25000, 150, "Benzin")) == false);
    assert(repo.get_Liste().size() == 12);

    assert(repo.suchen(1, "Logan").size() == 2);
    assert(repo.suchen(2, "Dacia").size() == 3);
    //assert(repo.loschen_auto(Auto("Logan", "Dacia", 2003, 321461, 500, 6, "GPL")) == true);
    assert(repo.get_Liste().size() == 11);
    assert(repo.suchen(1, "Logan").size() == 1);
    assert(repo.suchen(2, "Dacia").size() == 2);

    assert(repo.suchen(1, "Model S").size() == 1);
    assert(repo.suchen(2, "Tesla").size() == 1);
    //assert(repo.loschen_auto(Auto("Model S", "Tesla", 2021, 15000, 120000, 500, "Electric")) == true);
    assert(repo.get_Liste().size() == 10);
    assert(repo.suchen(1, "Model S").size() == 0);
    assert(repo.suchen(2, "Tesla").size() == 0);
}
