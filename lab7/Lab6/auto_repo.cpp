...
void Repo_auto::add_auto(Auto car, string fname)
{
    //wir fugen das Auto in der Liste
    int id = car.get_id();
    while (this->search_id(id) != -1)
        id++;
    car.setter_id(id);
    this->liste_autos.push_back(car);
    this->schreiben_im_datei(fname);
}
...
bool Repo_auto::loschen_auto(int id, string fname)
{
...
bool Repo_auto::bearbeiten_auto(Auto car, string fname)
{
    int id;
    ...
    for (int i = 0; i < this->liste_autos.size(); i++)
            ...
            && this->liste_autos[i].get_erstzulassungsjahr() == car.get_erstzulassungsjahr())
        {
            ...
            id = this->liste_autos[i].get_id();
            this->liste_autos[i] = car;
            this->liste_autos[i].setter_id(id);
            this->schreiben_im_datei(fname);
            return true;
        }
    return false;
...
void Repo_auto::lesen_von_datei(string file_name) {
    ...
    while (std::getline(datei_in, zeile))
    {
        ...
        vec.push_back(Auto(id, modell, marke, erstzulassungsjahr, km, preis, ps, treibstoff));
    }
    for(int i = 0; i < vec.size(); i++)
        this->add_auto(vec[i], file_name);
