...
bool UI_Auto::valid_int(string name) {
    if (cin.fail())          //der Benutzer gibt bei einem Integer ein String
    {
        ...
        cin.clear();         //wir l�schen was er uns gegeben hat
        cin.ignore(256, '\n');
        return false;
    }
    return true;
...
bool UI_Auto::valid_option(int option, int max_option) {
    ...
    if (option < 1 || option > max_option)     //der Benutzer gibt eine negative Zahl oder eine zu gro�e Zahl
    {
...
void UI_Auto::fav_list_add()
{
    ...
    this->kunde_controller.controller_fav_list_add(this->kunde_controller.get_Liste()[option - 1]);
...
void UI_Auto::show_list(vector<Auto> list) {
    ...
    {
        cout << i + 1 << ". " << list[i].get_marke() << " " << list[i].get_modell() << ":\n"
